...
class Database:
    ...
    """
    def __init__(self,username,password,host,port=None,db_type="postgres",database_name="experiment_log"):
        """
        ...
        if db_type == 'sqlite':
            ...
            DB_URL = 'postgres://' + username + ':' + password + '@' + host + ':' + str(port) + '/' + database_name
    ...
    def write_to_db(self, data_frame,username,password,host,port=None,db_type="postgres",database_name="experiment_log"):
        engine = self.__create_db(self.DB_URL)
        data_frame.to_sql(database_name, con=engine, if_exists='append')